...
def _create_analysis(client) -> int:
    """Upload an image and return its analysis_id."""
    resp = client.post(
        "/analysis/upload",
...
def _fetch_quiz_row(db_engine, analysis_id: int) -> Dict[str, Any]:
    """Read persisted answers_json directly."""
...
def test_legacy_payload_still_accepted(client):
    """Pre-Step-4 wire shape → 200 (new optional fields didn't become required)."""
...
def test_unknown_top_level_keys_are_ignored(client):
    """Pydantic extra=ignore: FE can ship new keys before BE learns of them."""
    analysis_id = _create_analysis(client)
    resp = client.post(
        "/quiz/submit",
        json={
...
def test_routine_level_no_produces_basic_plan_via_endpoint(client, db_engine):
    """Integration: routine_level=no → persisted RoutinePlan has the 3+2 basic schedule."""
    analysis_id = _create_analysis(client)
    payload = _full_extended_payload(analysis_id)
...
def test_sunscreen_rarely_never_adds_reminder_via_endpoint(client, db_engine):
    """End-to-end: rarely_never → persisted plan has "Every day" SPF reminder."""
    analysis_id = _create_analysis(client)
    resp = client.post(
        "/quiz/submit",
        json={
...
def test_404_when_analysis_id_unknown(client):
    """Negative path — unknown analysis_id → 404."""
    resp = client.post(
        "/quiz/submit",
        json={
            "analysis_id": 9999999,
